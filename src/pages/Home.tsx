...
import { getPortals as getSupabasePortals, migrateLocalStorageToSupabase } from "@/lib/supabaseStorage";
import { getPortals as getLocalPortals } from "@/lib/storage";
...
const Home = () => {
  ...
  const loadPortals = async () => {
    const stored = isAuthenticated ? await getSupabasePortals() : getLocalPortals();
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      ...
      const newPortals = isAuthenticated ? await getSupabasePortals() : getLocalPortals();
  ...
  return <Layout>
      ...
      {portals.length === 0 ? <div className="flex items-center justify-center min-h-[60vh]">
          <div className="text-center space-y-4 animate-fade-in">
            <p className="text-muted-foreground text-lg">No portals yet</p>
            {!isAuthenticated && (
              <p className="text-muted-foreground text-sm">
                Sign in to sync your portals across devices
              </p>
            )}
          </div>
